...
document.addEventListener("DOMContentLoaded", function () {
  ...
  initializeChat();
  initializeAppointmentForm(); // This will now correctly call the one at the end of the file
...
function initializeChat() {
  ...
  // This is the listener for the button you wanted fixed
...
// ===== START: UPDATED CHATBOT KNOWLEDGE BASE =====

// ===== START: UPDATED CHATBOT KNOWLEDGE BASE =====

// This is the expanded knowledge base you requested
const knowledgeBase = {
  // Greetings & Meta
  "hello,hi,hey":
...
// ===== END: UPDATED CHATBOT KNOWLEDGE BASE =====
...
// ===== BOOKING STATUS =====
function initializeBookingStatus() {
  loadBookings();
}
            ...
            <div class="booking-card">
                <div class="booking-header">
                    <div>
                        ...
                        <p style="color: var(--text-light); font-size: 0.875rem;">
                            ${
                              booking.appointmentType === "appointment"
                                ? "Regular Appointment"
                                : "Diagnosis & Tests"
                            }
                        </p>
                ...
                <div class="booking-details">
                    ...
                    <div class="booking-detail">
                        ...
                        <div class="booking-detail-content">
                            <p>Time Slot</p>
                            <p><strong>${booking.slot}</strong></p>
                        </div>
                    </div>
                </div>

                <div class="booking-actions">
                    <button class="btn btn-download" onclick="downloadBooking('${
                      booking.id
                    }')">
...
function downloadBooking(id) {
  ...
  const content = `
CHIKITSAMITRA - APPOINTMENT CONFIRMATION
========================================
...
2. Bring a valid ID proof and this appointment confirmation
3. Carry any previous medical records if applicable
4. In case of emergency, please call hospital helpline

Thank you for choosing ChikitsaMitra!
For queries, contact: support@chikitsamitra.in
...
Generated on: ${new Date().toLocaleString()}
  ...
  a.href = url;
  a.download = appointment-${booking.id}.txt;
  document.body.appendChild(a);
  a.click();
  window.URL.revokeObjectURL(url);
  document.body.removeChild(a);

  showToast("Appointment details downloaded!", "success");
}
...
// ===== APPOINTMENT FORM (This is the one, correct version) =====
function initializeAppointmentForm() {
  ...
  // START: NEW DYNAMIC DROPDOWN CODE

  // 1. Data for States and Districts
  ...
  // 2. Function to populate states
  function populateStates() {
    // Clear any existing options
    stateElement.innerHTML = '<option value="">Choose a state</option>';

    // Add states from our map
  ...
  // 3. Function to update districts based on state
  function updateDistricts() {
    const selectedState = stateElement.value;

    // Clear existing district options
    districtElement.innerHTML = '<option value="">Choose a district</option>';
  ...
  // 4. Initialize and set up event listener
  populateStates();
  updateDistricts(); // Call once to set initial (disabled) state
  stateElement.addEventListener("change", updateDistricts);

  // END: NEW DYNAMIC DROPDOWN CODE
  ...
  sendOtpBtn.addEventListener("click", () => {
    ...
    showToast(OTP sent to ${phone}: ${generatedOtp}, "success");
  });
  ...
  form.addEventListener("submit", (e) => {
    ...
    const formData = {
      ...
      // Add state and district to your saved data
      state: stateElement.value,
      district: districtElement.value,
      hospital: document.getElementById("hospital").value,
      department: document.getElementById("department").value,
      appointmentType: document.querySelector(
...
// The large commented-out block from your original file has been removed.