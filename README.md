...
- 🧘 **Inhale & Exhale**: A guided breathing exercise that rewards coins upon cycle completion.  
  *(File: `inhaleexhale.html`)*  
- 🕹️ **Tic-Tac-Toe**: A quick game against the AI, also linked to the reward system.  
  *(File: `tictactoe.html`)*
...
- **Client (Web App):**  
  `index.html`, `styles.css`, and `script.js` manage the UI, user interaction, and client-side validation.

- **Serverless API:**  
  Critical data (like appointment forms) is sent via `fetch` to the configured **APPS_SCRIPT_EXEC_URL**.

- **Data Persistence:**  
  - Booking status is managed via **Local Storage** for quick retrieval.  
...
```bash
git clone https://github.com/YourUsername/ChikitsaMitra.git
cd ChikitsaMitra
